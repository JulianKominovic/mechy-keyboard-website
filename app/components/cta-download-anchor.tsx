...
// Do a VERY ROBUST check to determine the OS. Prevent using deprecated APIs.
...
function getDownloadUrl(lastVersion: GHLatestVersionResponse | null) {
  let bestDownloadUrl: string | undefined =
    "https://github.com/JulianKominovic/mechy-keyboard/releases/latest";
  if (!lastVersion) {
  ...
  switch (os) {
    case "mac":
      bestDownloadUrl = lastVersion.assets.find((asset) =>
...
const CtaDownloadAnchor = ({ lastVersion }: Props) => {
  const os = getOS();
  const commingSoon = ["windows", "linux"];
...
export default CtaDownloadAnchor;
